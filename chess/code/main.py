...
class Game:
    ...
    # draw pieces and board notation
    def update_board(self):
        ...
        for i in range(len(self.white_pieces)):
            image_surf = pygame.transform.scale_by(self.white_surfs[self.white_pieces[i]], 5)
            image_rect = image_surf.get_frect(bottomleft=(self.board_rect.bottomleft[0] + self.white_position[i][0] * TILESIZE,
                                                          self.board_rect.bottomleft[1] - self.white_position[i][1] * TILESIZE))
            self.display_surface.blit(image_surf, image_rect)

        # draw black pieces
        for i in range(len(self.black_pieces)):
            image_surf = pygame.transform.scale_by(self.black_surfs[self.black_pieces[i]], 5)
            image_rect = image_surf.get_frect(bottomleft=(self.board_rect.bottomleft[0] + self.black_position[i][0] * TILESIZE,
                                                          self.board_rect.bottomleft[1] - self.black_position[i][1] * TILESIZE))
            self.display_surface.blit(image_surf, image_rect)


    # update pieces taken and turn indicator
    def update_ui(self):
        # draw rect for bg
    ...
    # update scoreboard keeping track of points
    ...
    def import_assets(self):
        ...
        # audio
        self.move_sound = pygame.mixer.Sound(join('..', 'audio', 'move.mp3'))
        self.capture_sound = pygame.mixer.Sound(join('..', 'audio', 'capture.mp3'))


    # draw possible moves indicator
    def draw_moves(self, moves):
        for move in moves:
            pos = ((move[0] * TILESIZE + TILESIZE/2), (self.board_rect.bottomleft[1] - move[1] * TILESIZE - TILESIZE/2))
            pygame.draw.circle(self.display_surface, (110, 135, 70), pos, 10)


    # draw menu for pawn promotion
    ...
    def get_moves(self, piece, pos, enemy=None, ally=None) -> list:
        if self.white_turn:
            ally_pos = self.white_position if ally is None else ally
            enemy_pos = self.black_position if enemy is None else enemy

            ally_pieces = self.white_pieces
        else:
            ally_pos = self.black_position if ally is None else ally
            enemy_pos = self.white_position if enemy is None else enemy
        ...
        if piece == 'pawn':
            ...
            try:
                if self.moves_list[self.move_count][0] == 'pawn':
                    ...
                    # if true, check if enemy pawn is next to selected pawn
                    if squares_moved == 2:
    ...
    # get possible moves for when king is checked
    def get_checked_moves(self, moves, piece_pos) -> list:
        valid_moves = []
        invalid_moves = []
    ...
    # check if a move will cause king to be in check
    def check(self, ally_pos=None, enemy_pos=None, enemy_pieces=None) -> bool:
        if self.white_turn:
        ...
        while self.running:
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    self.display_surface.blit(self.board_image, self.board_rect)
                    # assigns mouse pos to position on board
                    ...
                    if self.white_turn and not self.pawn_promotion:
                        if mouse_pos in self.white_position:
                            ...
                            piece_pos = mouse_pos
                            pygame.draw.rect(self.display_surface, (150, 180, 105),
                                             (mouse_pos[0] * TILESIZE, self.board_rect.bottomleft[1] - mouse_pos[1] * TILESIZE - TILESIZE, TILESIZE, TILESIZE))
                    ...
                    # check if piece is selected and if position of mouse is a possible move
                            ...
                            for move in moves:
                                if mouse_pos == move:
                                    ...
                                    if self.white_turn:
                                        ...
                                        # check if piece can capture enemy piece
                                    ...
                                    # play sound for move
                                    if self.moves_list[self.move_count][-1] == 'take':
                    ...
                    # selecting promoted piece when pawn is promoted
                    ...
                    # calculate possible moves of selected piece
                    if selected_piece:
                        last_selected_piece = selected_piece
                        moves = self.get_moves(selected_piece, piece_pos)
                        moves = self.get_checked_moves(moves, piece_pos)
                    else:
                        moves = []

                    # draw 'checked' indicator on check king
                    if self.king_checked:
                        if self.white_turn: king_pos = self.white_position[self.white_pieces.index('king')]
                        else: king_pos = self.black_position[self.black_pieces.index('king')]
                        pygame.draw.rect(self.display_surface,
                                         (233, 52, 43),
                                         (king_pos[0] * TILESIZE, self.board_rect.bottomleft[1] - king_pos[1] * TILESIZE - TILESIZE, TILESIZE, TILESIZE))

                    # highlight squares of moved piece
                    try:
                        pygame.draw.rect(self.display_surface,
                                         (150, 180, 105),
                                         (original_pos[0] * TILESIZE, self.board_rect.bottomleft[1] - original_pos[1] * TILESIZE - TILESIZE, TILESIZE, TILESIZE))
                        pygame.draw.rect(self.display_surface,
                                         (120, 150, 75),
                                         (self.last_move[0] * TILESIZE, self.board_rect.bottomleft[1] - self.last_move[1] * TILESIZE - TILESIZE, TILESIZE, TILESIZE))
                    except UnboundLocalError:
                        pass
                    ...
                    if self.king_checked:
                        ...
                        if any(len(self.get_checked_moves(self.get_moves(pair[0], pair[1]), pair[1])) != 0 for pair in pairs):
                            pass
                        else:
                            text_font = pygame.font.Font(None, 50)
                            text_surf = text_font.render('Checkmate', True, 'white')
                            self.display_surface.blit(text_surf, (700, 100))
                            text_font = pygame.font.Font(None, 40)
                            if self.white_turn:
                                text_surf = text_font.render('Black Wins', True, 'white')
                            else:
                                text_surf = text_font.render('White Wins', True, 'white')
                            self.display_surface.blit(text_surf, (720, 145))

                    # check for stalemate
                    if not self.king_checked:
                        if self.white_turn:
                            pairs = [(self.white_pieces[i], self.white_position[i]) for i in range(len(self.white_pieces))]
                        else:
                            pairs = [(self.black_pieces[i], self.black_position[i]) for i in range(len(self.black_pieces))]

                        if all(len(self.get_checked_moves(self.get_moves(pair[0], pair[1]), pair[1])) == 0 for pair in pairs):
                            text_font = pygame.font.Font(None, 50)
                            text_surf = text_font.render('Stalemate', True, 'white')
                            self.display_surface.blit(text_surf, (700, 100))
                            text_font = pygame.font.Font(None, 40)
                            text_surf = text_font.render('Draw', True, 'white')
                            self.display_surface.blit(text_surf, (745, 145))
